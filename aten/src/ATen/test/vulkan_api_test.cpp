...
TEST_F(VulkanAPITest, repeat_invalid_inputs_outputs_exceptions) {
  ...
  {
    const auto in_cpu = at::rand({3, 9, 11, 7, 3}, at::device(at::kCPU).dtype(at::kFloat));
  ...
  // Arrange: Number of dimensions of repeat dims can not be smaller than number of dimensions of tensor
  {
    const auto in_cpu = at::rand({3, 5, 11, 13}, at::device(at::kCPU).dtype(at::kFloat));
  ...
  {
    const auto in_cpu = at::rand({3, 9, 11, 7}, at::device(at::kCPU).dtype(at::kFloat));
    const at::IntArrayRef repeats = {5, 7, 3, 9, 2};
...
void test_repeat(const at::IntArrayRef input_shape, const at::IntArrayRef repeats) {
  c10::InferenceMode mode;
  ...
  bool check = true;
  for (int idx_input = 1; idx_input < input_shape.size() + 1; ++idx_input) {
    for (int idx_repeat = idx_input; idx_repeat < repeats.size() + 1; ++idx_repeat) {
      in_cpu = at::rand(input_shape.slice(0, idx_input), at::device(at::kCPU).dtype(at::kFloat));
      ...
      if (!local_check) {
        check = false;
        std::cout << "Repeat test failed when input is of shape " << input_shape.slice(0, idx_input) << " and repeat of " << repeat << std::endl;
        showRtol(out_cpu, out_vulkan.cpu());
      }
    }
  }
