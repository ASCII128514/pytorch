...
bool reduced_float_type_copy(bool requires_conj, TensorIteratorBase &iter) {
  ...
  return (!requires_conj && ((at::isReducedFloatingType(iter.dtype(1)) && iter.dtype(0) == kFloat &&
     sizeof(float) == strides_out[0] && (static_cast<int64_t>(elementSize(iter.dtype(1))) == strides_in[0] || strides_in[0] == 0)) ||
    (iter.dtype(1) == kFloat && at::isReducedFloatingType(iter.dtype(0)) &&
    static_cast<int64_t>(elementSize(iter.dtype(0))) == strides_out[0] && (sizeof(float) == strides_in[0] || strides_in[0] == 0))));
}

void reduced_float_copy_kernel(TensorIteratorBase &iter, bool requires_neg) {
  auto strides_out = iter.strides(0);
  auto strides_in = iter.strides(1);
  ...
  if ((iter.dtype(0) == kFloat) && at::isReducedFloatingType(iter.dtype(1))) {
  ...
  } else if (at::isReducedFloatingType(iter.dtype(0)) && (iter.dtype(1) == kFloat)) {
